...
GameWindow {
    ...
    Scene {
        ...
        // Sound effects (imported from SoundEffects.qml)
        SoundEffects {
            id: sfx
        }
                ...
                onTriggered: {
                    ...
                    if (ball.y > gameArea.height) {
                        ...
                        if (gameArea.lives <= 0) {
                            ...
                            ball.ballStuck = true
                            sfx.loseLife.play()                    // <-- lose life sound
                        }
                    }
                    ...
                    for (let i = 0; i < bricks.count; i++) {
                        let item = bricks.itemAt(i)
                         if (item && item.collides(ball.x, ball.y, ball.width, ball.height)) {
                            ...
                            // Play destroy or hit sound based on whether brick was destroyed
                            if (item.destroyed)
                                sfx.brickDestroy.play()          // <-- brick destroyed
                            else
                                sfx.brickHit.play()              // <-- brick damaged

                            // Check if all bricks destroyed
                            if (gameArea.checkLevelFinished()) {
                                gameArea.levelFinished = true
                                ball.ballStuck = true
                                sfx.levelComplete.play()           // <-- level complete sound
                            }
